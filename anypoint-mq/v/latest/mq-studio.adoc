...
http://www.mulesoft.org/schema/mule/anypoint-mq http://www.mulesoft.org/schema/mule/anypoint-mq/current/mule-anypoint-mq.xsd">
    ...
    <anypoint-mq:config name="Anypoint_MQ_Configuration" doc:name="Anypoint MQ Configuration">
        <anypoint-mq:provider url="https://mq-us-west-1.anypoint.mulesoft.com/api/v1" clientId="<ID>" clientSecret="<SECRET>"/>
    </anypoint-mq:config>
...
----
|messageId |(Publish operation only) Optional ID of a message to publish. For FIFO use,
if you specify a Message ID and the ID is the same on multiple messages, the
messages with the same Message ID are overwritten. Normally this field is not set
and the message ID value is auto-generated. Unlike standard queues, FIFO queues do not introduce duplicate messages. FIFO queues help you avoid sending duplicates to a queue. If you retry sending a message  with the same message ID within the 5-minute deduplication interval, Anypoint MQ FIFO does not introduce any duplicates into the queue. You can set a message ID in Anypoint Studio. If a message ID is not set in Studio, MQ auto generates a unique message ID for each message that's sent to a queue.
